...
def write_overall_results(res: dict, data: dict, tag: str):
    ...
    """
    table = PrettyTable()
    table.field_names = ["Number of cities", "Tour distance QLearning", "Best distance", "%", "Tiempo"]
    for c in res:
        table.add_row([c, res[c][0], data[int(c[:2])][1], round(data[int(c[:2])][1]/res[c][0] * 100,4), round(res[c][1],4) ])
