...
def eps_greedy_update(nodoinit, carinit, Q_table, mask, maskcars, route, cars):
    ...
    for i in range(1, dimension):
        ...
        if len(possible) == 1 and len(possiblecars) == 0 and limitbonus > currentbonus:
            ...
            Q_table[currentcar, current, next_visit] += LEARNING_RATE * (reward + GAMMA * Q_table[currentcar, next_visit, nodoinit]- Q_table[currentcar, current, next_visit])
            # Avanza
            current = next_visit
            mask[current] = False
        ...
        # Sólo queda un nodo, hay coches libres y todavía falta bonificación
            ...
            if next_car != currentcar:
                ...
                Q_table[currentcar, current, next_visit] += LEARNING_RATE * (reward + GAMMA * Q_table[next_car, next_visit, nodoinit]- Q_table[currentcar, current, next_visit])
                current = next_visit
        ...
        # Hay nodos y no hay coches libres, pero aún falta bonificación
            ...
            if limitbonus <= currentbonus:
                ...
                maskcars[currentcar] = True
                _, bestcar, max_next = currmax(Q_table, currentcar, next_visit, mask, maskcars)
                maskcars[currentcar] = False
                Q_table[currentcar, current, next_visit] += LEARNING_RATE * (reward + GAMMA * Q_table[currentcar, next_visit, bestcar]- Q_table[currentcar, current, next_visit])
                current = next_visit
                mask[current] = False
        ...
        # Quedan nodos y coches libres, y la bonificación aún no se alcanza
            ...
            if currentbonus >= limitbonus:
                ...
                Q_table[next_car if next_car!=currentcar else currentcar, current, next_visit] += LEARNING_RATE * (reward + GAMMA * Q_table[next_car if next_car!=currentcar else currentcar, next_visit, nodoinit] - Q_table[next_car if next_car!=currentcar else currentcar, current, next_visit])
                # Cierre de ruta
                route.extend([next_visit, nodoinit])
                cars.extend([next_car, next_car])
                # Q retorno
                ...
                if next_car == currentcar:
                    ...
                    _, bestcar, max_next = currmax(Q_table, next_car, next_visit, mask, maskcars)
                    maskcars[next_car] = False
                Q_table[next_car if next_car!=currentcar else currentcar, current, next_visit] += LEARNING_RATE * (reward + GAMMA * Q_table[next_car if next_car!=currentcar else currentcar, next_visit, bestcar] - Q_table[next_car if next_car!=currentcar else currentcar, current, next_visit])
                current = next_visit
                pointchangecar = next_visit
                mask[current] = False
                route.append(next_visit)
...
def evaluar_multiples(Q_table, runs=10):
    ...
    for _ in range(runs):
        start = time.time()
        costo, ruta, autos = solving_sarsa(Q_table.copy())
        t = time.time() - start
...
with open("resultadoss.txt", "a", encoding="utf-8") as f:
    print(file=f)  # línea vacía de separación
    print(name + " Sarsa", file=f)
